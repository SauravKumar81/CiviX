...
import { Camera, MapPin, ChevronLeft, Send, Info, Trash, type LucideIcon } from 'lucide-react';
...
const ReportIssuePage: React.FC = () => {
  ...
  const handleDetectLocation = () => {
    ...
    setTimeout(() => setShowLocationCheck(false), 1500);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      await createReport({
        title: title || `Report - ${category}`,
        description,
        category,
        status: 'pending',
        location: {
          type: 'Point',
          coordinates: [-122.3321, 47.6062], // Placeholder for now
      ...
      <main className="max-w-3xl mx-auto p-6 space-y-8">
        ...
        <section className="space-y-6 bg-white dark:bg-gray-900 p-8 rounded-[32px] border border-gray-100 dark:border-gray-800 shadow-sm transition-colors">
          <div className="space-y-4">
            <label className="text-sm font-black text-gray-900 dark:text-gray-200 uppercase tracking-widest block transition-colors">Report Title (Optional)</label>
            <input 
              type="text"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
              placeholder="e.g. Broken Pothole on Main St" 
              className="w-full p-5 bg-gray-50 dark:bg-gray-900 border-2 border-transparent rounded-3xl focus:bg-white dark:focus:bg-gray-800 focus:border-primary outline-none transition-all text-gray-900 dark:text-white font-medium placeholder:text-gray-400"
            />
          </div>
          ...
          <div className="space-y-4">
            ...
            <textarea 
              ...
              className="w-full min-h-[160px] p-5 bg-gray-50 dark:bg-gray-900 border-2 border-transparent rounded-3xl focus:bg-white dark:focus:bg-gray-800 focus:border-primary outline-none transition-all text-gray-900 dark:text-white font-medium placeholder:text-gray-400"
            />
          </div>
        ...
        <section className="space-y-6 bg-white dark:bg-gray-900 p-8 rounded-[32px] border border-gray-100 dark:border-gray-800 shadow-sm transition-colors">
          <div className="flex justify-between items-center">
            ...
            <button 
              ...
              className="text-xs font-black text-primary flex items-center gap-1 hover:underline transition-all active:scale-95"
            >
          ...
          <div className="relative h-64 bg-gray-100 dark:bg-gray-800 rounded-[28px] overflow-hidden border border-gray-100 dark:border-gray-700 group">
            {/* Fake Map Background */}
            <div className="absolute inset-0 grayscale contrast-125 opacity-40 bg-[url('https://api.mapbox.com/styles/v1/mapbox/light-v10/static/-122.3321,47.6062,12/800x600?access_token=pk.placeholder')] bg-cover transition-all group-hover:scale-110 duration-1000" />
            ...
            <div className="absolute bottom-4 left-4 right-4 bg-white/95 dark:bg-gray-900/95 backdrop-blur-sm p-4 rounded-2xl flex items-center gap-3 border border-white dark:border-gray-800 transition-all shadow-lg group-hover:translate-y-[-4px]">
        ...
        <button 
          ...
          disabled={loading}
          className="w-full h-16 bg-primary hover:bg-blue-700 text-white font-black rounded-[24px] shadow-2xl shadow-primary/30 flex items-center justify-center gap-3 transition-all hover:scale-[1.01] active:scale-[0.99] group mt-12 mb-20 disabled:opacity-50"
        >
...
const CategoryCard = ({ label, icon: Icon, active = false, onClick }: { label: string, icon: LucideIcon, active?: boolean, onClick: () => void }) => (
  <button 
    onClick={onClick}
    className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-3 transition-all ${active ? 'bg-blue-50 dark:bg-blue-900/20 border-primary' : 'bg-white dark:bg-gray-900 border-gray-100 dark:border-gray-800 hover:border-gray-200 dark:hover:border-gray-700'}`}
  >
