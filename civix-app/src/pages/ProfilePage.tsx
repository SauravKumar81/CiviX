...
import { getPublicProfile, updateProfile, getBookmarks } from '../services/authService';
  ...
  return (
    <div className="min-h-screen bg-[#0B1416] text-[#D7DADC] font-sans pb-20">
      ...
      <div className="w-full h-14 bg-[#1A282D] border-b border-[#27353B] flex items-center px-4 sticky top-0 z-40">
        <button onClick={() => navigate('/feed')} className="flex items-center gap-2 hover:bg-[#27353B] px-3 py-1.5 rounded-full transition-colors text-sm font-bold">
      ...
      <div className="max-w-[1200px] mx-auto pt-8 px-4 flex flex-col lg:flex-row gap-6">
                     ...
                     return filteredActivities.map(act => (
                        <div key={act.id} className="bg-[#1A282D] p-4 rounded-xl border border-[#27353B] flex gap-4 transition-colors hover:border-[#3A4A51] cursor-pointer" onClick={() => navigate(`/report/${act.report._id}`)}>
                           <div className="flex flex-col items-center gap-1 w-8 pt-1">
                             ...
                             {act.type === 'saved' && <Bookmark size={16} className="text-violet-500 fill-violet-500" />}
                           </div>
                           <div className="flex-1 min-w-0">
                              <div className="flex items-center gap-2 mb-1 text-xs text-gray-400 font-medium">
                                 ...
                                 {act.type === 'saved' && <span className="text-violet-400 font-bold">You saved this</span>}
                                 <span>•</span>
        ...
        <div className="w-full lg:w-[320px] shrink-0">
           <div className="bg-[#1A282D] rounded-xl border border-[#27353B] overflow-hidden sticky top-20">
              ...
              {/* Banner Area */}
              <div className="h-20 bg-gradient-to-br from-violet-900 via-indigo-900 to-[#1A282D] relative">
              ...
              <div className="p-4 pt-10 relative animate-in fade-in duration-300">
                 ...
                 <h2 className="text-lg font-black text-white leading-tight">{profileUser?.name || 'Citizen'}</h2>
                 <p className="text-xs text-gray-400 font-medium mb-3">u/{profileUser?.username || 'user'}</p>
                   ...
                   <button 
                     ...
                     className="w-full py-1.5 mb-4 rounded-full font-bold text-sm border border-[#D7DADC] text-[#D7DADC] hover:bg-[#27353B] transition-colors flex justify-center items-center gap-2"
                   >
                 ...
                 <div className="grid grid-cols-2 gap-y-3 gap-x-4 mb-4">
                    ...
                    <div>
                      <div className="text-sm font-bold text-white mb-0 mt-1 flex items-center gap-1">
                 ...
                 {/* Achievements */}
                 <div className="mb-4 pt-4 border-t border-[#27353B]">
                    <h3 className="text-xs uppercase font-bold tracking-wider text-gray-500 mb-2 block">Achievements</h3>
                    <div className="flex gap-2 items-center">
                       ...
                       <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-violet-500 to-purple-500 flex items-center justify-center p-1 shadow-md shadow-purple-500/20" title="Feed Finder">
                 ...
                 {isOwnProfile && (
                   <div className="pt-4 border-t border-[#27353B] space-y-3">
                      <h3 className="text-xs uppercase font-bold tracking-wider text-gray-500 mb-1 block">Settings</h3>
                      ...
                      <button onClick={logout} className="w-full mt-2 flex items-center justify-center gap-2 py-1.5 border border-red-500/20 text-red-400 hover:bg-red-500/10 rounded-xl text-sm font-bold transition-colors">
          ...
          <div className="bg-[#1A282D] w-full max-w-md rounded-2xl p-6 shadow-2xl border border-[#27353B] animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleEditSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" value={editForm.name} onChange={(e) => setEditForm({...editForm, name: e.target.value})} className="w-full p-3 bg-[#0B1416] text-white rounded-xl border border-[#27353B] focus:border-violet-500 outline-none transition-colors" />
              </div>

              <div>
                <label className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-1 block">Username</label>
                <input type="text" value={editForm.username} onChange={(e) => setEditForm({...editForm, username: e.target.value})} className="w-full p-3 bg-[#0B1416] text-white rounded-xl border border-[#27353B] focus:border-violet-500 outline-none transition-colors" />
              </div>
              
              <div>
                <label className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-1 block">Bio</label>
                <textarea value={editForm.bio} onChange={(e) => setEditForm({...editForm, bio: e.target.value})} className="w-full p-3 bg-[#0B1416] text-white rounded-xl border border-[#27353B] focus:border-violet-500 outline-none h-24 resize-none transition-colors" placeholder="Tell us about yourself..." />
              </div>

              <div>
                <label className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-1 block">Location</label>
                <input type="text" value={editForm.location} onChange={(e) => setEditForm({...editForm, location: e.target.value})} className="w-full p-3 bg-[#0B1416] text-white rounded-xl border border-[#27353B] focus:border-violet-500 outline-none transition-colors" placeholder="City, State" />
              </div>

               <div>
                <label className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-1 block">Avatar URL or Upload</label>
                <div className="flex gap-2 items-center">
                   <input type="text" value={editForm.avatar} onChange={(e) => setEditForm({...editForm, avatar: e.target.value})} className="flex-1 p-3 bg-[#0B1416] text-white rounded-xl border border-[#27353B] focus:border-violet-500 outline-none transition-colors text-sm" placeholder="https://..." />
